...
- `yarn serve` — start Vue dev server.
- `yarn build` — build pages into `docs/`.
- `yarn lint` — run ESLint per `.eslintrc.js`.
- `yarn start` — serve `docs/` via Express (`HOST`, `PORT`, `HTTPS`).
- `yarn dev` / `yarn dev:inspect` — Express with hot reload/inspect.
- Data tools: `yarn cube3-gentable6`, `cube3-gensolvermap`, `cube3-genhash` (outputs under `static/`).

## Coding Style & Naming Conventions
- TypeScript + Vue 3. Use tabs for indentation, double quotes, and semicolons.
...
- Ensure `yarn build` and `yarn lint` pass; update docs when structure or commands change.
