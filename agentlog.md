...
- Literal zero-omission was not viable and would have inverted a setting: `c=0`/`g=0` mean "off" while both default to on, so dropping them would read back as on. Omitting *defaults* instead covers every zero that actually occurs (`cx=0`, `cy=0`) plus `w=8` and `b=1`, and loses nothing.
- Once a field can be absent, absence has to have one fixed meaning, and it has to be "the default" rather than "whatever this browser last had" — otherwise a link with `cx` omitted lands the recipient on the centre left in their own `localStorage` instead of the origin. So a query naming any of our keys is now authoritative and its absent keys read as defaults; only a URL carrying none of them (bare, or foreign keys only) falls back to stored state, which is what a returning visitor arrives with. Nothing is lost by this, because every non-default field is written to the URL by construction.
- Two places had to stop keying off "which fields are present":
  - `syncQuery` rebuilds its own keys from scratch and merges foreign keys back, rather than spreading the current query. Spreading would leave a field that returned to its default lingering at its stale value, since a plain merge cannot express a removal.
  - The `$route` guard compares over the fixed key set instead of `urlQuery`'s keys. A field at its default is absent from both sides, and one that just became default is absent from only one — checking only the keys `urlQuery` still has would miss exactly that transition, which is the back-from-a-panned-view case.
- Fixed on the way: `encodeNumber(0)` took the exponential branch and emitted `0.00000000e+0`, so the default centre alone was 30 characters of encoded noise in every URL.
- Verified: 31 assertions with a real clock over CDP — the formula is always present and alone in an all-default view; each of the other six fields appears exactly when it differs and vanishes when it returns to its default (checked field by field, and checked gone from the address bar, not just from the route object); a foreign key survives our rebuild; going back from a panned view returns to the origin and the default width; a partial shared URL is fully determined by the URL rather than merged with the recipient's stored brightness and flags; a hand-typed URL with no `f` still renders and gets `f` written back; and a URL with none of our keys still restores stored state.
- A default view's query went from 73 characters to 17. `yarn lint` 0 errors, `yarn build` passed, `tests/complexExpression.ts` clean.

## 2026-09-03 complex-function: leading-edge rebuild limit, gated on the compiled shader body

- Measured the input latency before changing anything, over CDP with a real clock: parse + GLSL codegen 0.1 ms, `updateExpression` (build `ShaderMaterial` + `renderer.compile()`) 7-17 ms, first frame to pixels 1-2 ms — 2-20 ms total, and flat across expression complexity because the formula compiles to straight-line GLSL. The wait was entirely the 2 s debounce, three orders of magnitude above the render cost.
  - `--virtual-time-budget` freezes `performance.now()`, so the first attempt measured all zeros. Timing anything in headless Chrome needs CDP (`--remote-debugging-port` + `Runtime.evaluate`) rather than virtual time.
- Two defects in the old input path, both real:
  - It was trailing-edge, so the *first* edit of a burst also waited the full delay. Now leading-edge: the first change applies at once and further changes inside a 600 ms window are held, the window reopening if one was held. Continuous typing rebuilds once per window instead of once per keystroke, and no edit ever waits when the window is closed.
  - It started a timer unconditionally, whether or not the input parsed and whether or not the result differed. Now every keystroke is compiled (0.1 ms — cheap enough to spend on input that may be garbage) and only a parse that yields a shader body different from the one on the GPU is rate limited. Unparseable input and cosmetic edits (`z ^ 2` for `z^2`, re-typing the same formula) start no timer and do no work.
- Error reporting is now a separate 2 s timer rather than a side effect of the rebuild delay, which is what lets the rebuild window be short: `(1+z^` still never flashes an error on the way to `(1+z^2)^-1`, but a formula left broken is reported instead of sitting there silently. `shaderBody` records what the GPU is running so the input path can distinguish a real change from a cosmetic one.
- `clearExpressionTimer` now also drops the held expression and the pending error, and preset/favourite clicks and URL adoption call it — otherwise a keystroke held a moment earlier would land on top of an explicit pick a moment later.
- Verified: 35 assertions with a real clock over CDP, counting actual rebuilds by wrapping `updateExpression` — first edit applies in ~13 ms; a held edit lands when the window closes; 8 keystrokes that are valid at *every* step over 500 ms produce 2 rebuilds, not 8; unparseable input opens no window and reports only after the delay while the last good plot stays up; whitespace-only and repeat edits rebuild nothing; Enter bypasses an open window with no stale value landing after; a preset cancels both a held keystroke and a pending error report.
  - One assertion initially passed for the wrong reason: every prefix of `gamma(z)` except the last is unparseable, so "1 rebuild for 8 keystrokes" measured the parse gate, not the rate limit. Added a case where every intermediate step compiles.
- URL and persistence paths re-checked after the change (12 assertions), `tests/complexExpression.ts` clean, `yarn lint` 0 errors, `yarn build` passed.
...
- **Primary: service worker** (`public/asset-cache-sw.js`, copied to `docs/` root by `viteStaticCopy`). Cache-first, driven by a declarative `RULES` table at the top of the file — each rule matches by `url.pathname` regex and gets its OWN cache bucket `klstudio-asset-<name>-<version>` (bump a rule's `version` to invalidate just that bucket). Rules cover the fixed-path, non-fingerprinted, cross-session assets audited from `public/`: `soundfont` (gm.sf3 / `.sf3`/`.sf2`, ~38 MB), `chess-openings` (`chess/open-games/[3-7].yaml`, ~17 MB), `fluidsynth` (`fluid/libfluidsynth-*.js`, ~2.3 MB), `opencv` (`opencv(.min).wasm`, ~1.2 MB). Deliberately excludes Vite-bundled `assets/*-<hash>.js` (stockfish.js, opencv.js) — those are fingerprinted and cached by URL already; intercepting them is risky. Only configured same-origin GETs are intercepted; everything else (navigation, hashed JS/CSS) falls through untouched, so hash-routing is unaffected. `skipWaiting` + `clients.claim`; `activate` prunes stale/removed asset buckets AND the legacy `klstudio-soundfont-*` caches from the earlier soundfont-only SW. **Key fix (carried over):** wrap `cache.put` in `event.waitUntil` — a fire-and-forget put of a 38 MB body is dropped when the worker terminates after the fetch event, leaving the cache empty.
- **Registration** (`app/inc/registerAssetCacheSW.ts`): guarded (`"serviceWorker" in navigator`), idempotent (single cached promise), registered at `${import.meta.env.BASE_URL}asset-cache-sw.js` with `scope: BASE_URL` so its default scope covers every asset dir on a subpath host (GitHub Pages) with no `Service-Worker-Allowed` header. Errors logged + swallowed. Called from `fluidAudio.ts` before the fetch, then `serviceWorker.ready` raced against a 1.5 s timeout so a slow/failed activation never stalls playback (first visit still fetches from network; legacy MIDI.js piano stays the fallback). Once installed by any audio visit the SW persists and serves ALL rules on later visits (chess YAML, opencv wasm), not just the soundfont.
- **Backup: HTTP Cache-Control** (`main.ts`): 30-day `maxAge` static mounts for `/soundfont`, `/fluid`, `/chess/open-games`, `/opencv.min.wasm` (mirroring the SW rules) before the root mount, for self-hosted/Express deployments. Bounded max-age (not immutable) since filenames are unversioned. On GitHub Pages this layer doesn't run; the SW is the fix there.
- Verified with headless Chrome on the Express server (localhost = secure context): SW registers + controls the page; all four assets fetch with correct byte counts and land each in its OWN bucket (`klstudio-asset-{soundfont,chess-openings,opencv,fluidsynth}-v1`); soundfont reload served from `cache-storage` (`transferSize: 0`); `/index.html` and hashed `assets/*` are NOT cached; no legacy `klstudio-soundfont-*` bucket remains. `curl -I` confirmed `Cache-Control: public, max-age=2592000` on all four paths. `yarn lint` 0 errors, `yarn build` emits `docs/asset-cache-sw.js` at root. (Superseded the initial soundfont-only `soundfont-sw.js` / `registerSoundfontSW.ts`, both removed.)
...
- **Capability-based gating, not viewport width.** The compact layout and hint-swap are gated on `@media (hover: none) and (pointer: coarse)`, so a narrow desktop window keeps the mouse layout and a touch phone gets the finger layout at any width. The gesture handlers are likewise gated at runtime on `event.pointerType === "touch"`, so mouse paths are untouched. Verified: at 1280×800 the layout stays `display: block`, 2-col palette, `max-height: 70vh` board, desktop hints shown; at 390×844 touch emulation it's a fixed-height flex column with touch hints.
- **No-scroll layout.** `.hexiamond` → `box-sizing: border-box; display: flex; flex-direction: column; height: 100dvh; overflow: hidden`. Board panel and board both `flex: 1 1 0; min-height: 0` so the SVG (scaled by `viewBox`) absorbs all leftover space; palette `flex: 0 0 auto; max-height: 34vh` compacted to a 4-column grid with smaller thumbnails and `h2`/keyboard hints hidden. A `box-sizing: border-box` fix was needed — content-box + `.4rem` padding overflowed `100dvh` by ~13px and clipped the palette. Headless-Chrome verify: `scrollHeight == innerHeight` (844), palette and board both fully in viewport.
- **Touch gestures on placed blocks.** Extracted `reorientHovered` into `applyReorient(blockId, map, action, silent?)` (explicit block id + a `silent` mode that skips the history commit during live stepping). One finger on a block starts a normal move/snap and arms a 500 ms long-press timer → **flip** (`graph.mirror`); a move past 4px cancels it. A second finger begins a **two-finger twist**: accumulate signed shortest-arc angle deltas and step `graph.cw`/`graph.ccw` every `π/4`, re-snapping in place; lifting either finger commits once. Second finger is captured via a new board `@pointerdown="boardPointerDown"`. All state cleaned in `clearBlockPointer`/`beforeUnmount`.
- Validation: `yarn lint` 0 errors (12 pre-existing warnings elsewhere), `yarn build` OK (`hexiamond` chunk), headless-Chrome (puppeteer-core, 390×844 touch emulation) confirmed no-scroll + synthetic twist/long-press both change `orientationId`; desktop regression checked at 1280×800.

## 2026-07-20 Hexiamond standard board — exhaustive solution census & per-position frequency graphs

- Exhaustively traversed every solution of the `std` hexiamond board (72 cells, 12 blocks) with a fresh exact-cover search: `tools/hexiamond-std-frequency.ts`. Each placement's cell set is a `BigInt` bitmask; the recursion picks the uncovered cell with the fewest live candidates (MRV) and dead-ends the instant any cell becomes uncoverable. Result: **5,885 distinct solutions**, 1,855 candidate placements, 1,785,485 nodes (~45 s).
- **Orientation-model correction.** A first pass built the shape via `geometry.ts::buildShape` and got 11,770 — exactly 2× the true count. Cross-checked against the reference Python in `~/data/HexagonBlocks.7z` (`algebra_utils.py`/`traverser.py`), which counts 5,885. Root cause: `makeOrientations` auto-adds a mirrored chirality (`reflectPoint`) to *every* piece, but the reference only rotates (`patterns × spin`) and encodes chirality in the data (flippable pieces list both chiralities as two patterns). The **Bar** is chiral yet listed with a single pattern (spin 3) → the reference treats it as one-sided (3 orientations, 54 placements); auto-reflection wrongly doubled it (6 orientations, 108 placements), doubling every solution. Every other block was unaffected. The tool now builds orientations rotation-only (`buildReferenceShape`), matching the Python placement counts and 5,885 exactly.
- For every block it tallies how often each distinct *position* (placement = orientation + translation) appears across all solutions, not per-cell coverage. Sanity check passes: each block's position-frequency sum equals the solution count (5,885), since every block is used exactly once per solution. Written to `static/hexiamond-std-frequency.json`.
- `tools/hexiamond-std-frequency-html.ts` renders the 12 graphs into one HTML file (`static/hexiamond-std-frequency.html`). Each graph reuses the app's `outlinePath`/`triangleVertices`/`viewBoxForPoints` geometry: a faint board grid, then every used position of one block drawn as a **frame line** in the block color — stroke opacity and width both scale with frequency, and frames are stroked in ascending count order so the most frequent positions sit on top. Hot spots read clearly (Ball's apex hexagon 1,430×, Dart 1,043×, Bar 943×).
