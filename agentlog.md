- **Ground as real S³ geometry.** The old flat `GridHelper` didn't go through the S³ transform, so with the origin-pinned camera it looked welded to the screen. Replaced with a grid of points on an S³ great-2-sphere patch below the camera start (built once via the exponential map `expMapS3`), stored as fixed 4-vectors and **re-projected per-vertex every frame** through the same near-image mapping the puppet uses (`bearingTo` → `dir·θ·VIEW_SCALE`). It flows and curves with camera motion. Kept a small angular patch (half-extent 1.2 rad, θ < π everywhere) so it needs no far/wrap image. It's a **fixed landmark** in S³: you can walk off it, and it reappears when you loop back around. Rendered as `LineSegments`; verified in-browser that its pixels shift/recede with movement instead of staying fixed.
- **Moving camera through S³ (`app/s3/camera.ts`).** Replaced the fixed in-place camera with a full **SO(4) frame** — an orthonormal basis `{right, up, fwd, pos}` of 4-vectors, `pos` being the eye on S³. Every motion is a Givens rotation of two frame vectors: `moveForward` rotates the `(pos, fwd)` pair (glide the faced geodesic; 2π returns), `strafe` the `(pos, right)` pair, `yaw`/`pitch` the tangent frame (eye fixed). Rendering projects the puppet's fixed S³ point onto the frame: the `pos` component gives `cos θ` (geodesic distance), the tangent components give the camera-local bearing `dir`. **Both images are recomputed from this bearing every frame** — near along `+dir` at `θ`, far along `−dir` at `2π−θ` — so as the camera translates, the two images' positions relative to it update correctly (no longer glued to fixed world directions). `orthonormalize()` (Gram-Schmidt) sheds float drift.
- Controls: **WASD** = geodesic move (forward/back + strafe), **drag** = yaw/pitch look, **scroll** = glide forward. The three.js camera stays at the origin looking down −z; all S³ motion is baked into the image transforms.
- Verified: `tests/s3Camera.ts` — moveForward reduces then (past the point) grows distance and flips the puppet behind, a full 2π loop returns, yaw/strafe shift the bearing correctly, and the frame stays orthonormal (SO(4)) after mixed motion. In-browser: holding W loomed the near image 13%→96% (closest approach), collapsed to ~19% passing through, then receded to ~5% behind — and continued travel looped the puppet back to the front. Focused ESLint clean, `yarn build` emits the `s3-simulator` chunk.

## 2026-07-26 S³ simulator — flat projection with S³-adjusted world transform (near + far images)

- New paradigm for the S³ simulator: render with an ordinary flat `PerspectiveCamera` but bake all curvature into each object's world matrix. Intra-mesh bending is ignored (rigid meshes); only the object *center* is transported through the hypersphere. Replaces the earlier abandoned sky-sphere ray-tracer and spherical-inversion copy.
- Light joins camera and object along a great circle, travelling both ways, so every object renders as **two images**: a **near** image (short arc `θ`, upright, orientation-preserving) and a **far** image (long arc `2π−θ`). Working the great-circle Jacobian: the far image's linear map is a full **point inversion** `−I = diag(−s,−s,−1)` — a 180° rotation about the line of sight (**upside-down** when you turn to face it: feet-up, matching "light from the feet returns from the top") *and* a depth reversal. `det < 0`, so the far mesh is drawn `THREE.BackSide` to keep its outer surface toward the viewer. (Earlier code applied `BackSide` but omitted the inversion, so the far puppet was wrongly upright; fixed.)
- `app/s3/hyperTransform.ts`: pure `imageProjection(arc, R, far)` (no THREE) → `{renderDistance, tangentialScale, apparentSize=1/sin θ, …}`, and `buildImageMatrix(...)` composing `T(dist·dir)·Frame·diag(tan,tan,1)·Frameᵀ·orientation`. Render distance uses the geodesic *angle* (0..π), keeping the embedded scene compact while preserving the `1/sin θ` size law; a distance-only `VIEW_SCALE` frames the puppet without touching the size law.
- `app/views/s3-simulator.vue`: near group = the puppet's own group; far group = a geometry-sharing clone with a `BackSide` material. Both `matrixAutoUpdate=false`. The two images are placed at **fixed world directions** (near at −Z, far at +Z, diametrically opposite) and a single **free-look camera** sits at the S³ origin, rotating in place (drag = yaw/pitch, scroll = travel the great circle). This lets you turn between the two images by view angle rather than having them glued to the gaze. Lights are parented to the camera so both images stay lit as you turn. Aspect-ratio/pixel-ratio resize fix retained.
- Fixed: three r0.126 `OrbitControls` has no `getDistance()` (an earlier orbit-based iteration); replaced entirely by the in-place free-look camera.
- **Sky/fog depth cue:** background clear color and `THREE.Fog` both use one sky color (`0x8cb3d9`). Because render depth is proportional to great-circle arc distance, plain linear depth fog *is* fog-by-S³-distance. `fogFar` is chosen so a full-circumference light path (arc = 2000π → render depth `2π·VIEW_SCALE`) blends in exactly **70% sky color** (`fogFar = depth / 0.70`); the antipode (1000π) sits at 35%, the eye at 0%. A dark ground grid (`GridHelper`, 140 units) sits at y=−2.5 and fades into the fog horizon. Verified numerically (70.0% at 2000π) and in-browser.
- Verified: `tests/s3HyperTransform.ts` asserts the round-trip size schema for R=1000 (shrink 0→500π, grow 500π→1000π, merge at the antipode, shrink 1000π→1500π, grow 1500π→2000π; equator minima; near/far size always equal) **and** the far-image inversion (near det>0, far det<0, local +Y→world −Y = upside-down). Focused ESLint clean, `yarn build` emits the `s3-simulator` chunk. Headless-Chrome checks confirmed two lit lobes 180° apart and the ground grid rendering.
